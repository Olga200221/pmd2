...
class MyGLRenderer(private val context: Context) : GLSurfaceView.Renderer {
    ...
    // Новый параметр: выбранная планета
    var selectedPlanetIndex: Int = 0

    // Куб для подсветки выбранной планеты
    ...
    override fun onSurfaceCreated(gl: GL10?, config: EGLConfig?) {
        ...
        // Куб создаём один раз, размер 1 (масштабируем потом)
    ...
    override fun onDrawFrame(gl: GL10?) {
        ...
        for (i in planets.indices) {
            ...
            // Если планета выбрана, рисуем прозрачный куб вокруг неё
    ...
    // Метод для рисования прозрачного куба вокруг выбранной планеты
    private fun drawSelectionCube(planetIndex: Int) {
        val planetSize = planetSizes[planetIndex]
        val planetDistance = planetDistances[planetIndex]
        val cubeScale = planetSize * 1.8f

        val model = FloatArray(16)
        Matrix.setIdentityM(model, 0)
        model.rotate(orbitTilt, 1f, 0f, 0f)
        model.rotate(planetAngles[planetIndex], 0f, 1f, 0f)
        model.translate(planetDistance, 0f, 0f)
        model.scale(cubeScale, cubeScale, cubeScale) // масштабируем куб

        // Цвет куба с прозрачностью
