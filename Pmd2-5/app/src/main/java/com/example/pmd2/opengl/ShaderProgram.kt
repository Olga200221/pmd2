...
object ShaderProgram {
    ...
    // Убрали автоматическое создание в init — теперь вызываем вручную из onSurfaceCreated

    // Публичный метод для создания/пересоздания стандартного шейдера
    ...
    fun drawSpherePhong(
        ...
        // ─── ИСПРАВЛЕНИЕ: источник света в пространстве модели (чтобы блик двигался) ─────
        val lightWorldPos = floatArrayOf(5f, 5f, 5f, 1f)  // позиция света в мировых координатах
        ...
        if (Matrix.invertM(invModelMatrix, 0, modelMatrix, 0)) {
            ...
            // Если инверсия не удалась (редко, но на всякий случай)
